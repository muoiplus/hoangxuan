...
class JQArchiveList extends WP_Widget
{
    ...
    protected function buildHtml($instance)
    {
        ...
        for ($i = 0; $i < count($years); $i++) {
            ...
            if(strpos($currentUrl,'category')!=false && strpos($currentUrl,'postyear')==false){
                $expandYear = false;
            }else{
            ...
            if($instance['only_sym_link']){
                $html.= "\n<li class=\"{$class}\">";
                $html.= "<a href=\"{$yearLink}\"> {$years[$i]->year}";
            } else {
...
//                $html.= " ({$years[$i]->posts})";
            ...
            $html.= "</a><a class=\"jaw_years\" href=\"{$yearLink}\"><span class=\"jaw_symbol\">{$symbol}</span></a><ul>";
            ...
            foreach ($months as $month) {
                ...
                $currentUrl = $_SERVER["REQUEST_URI"];
                $paramCurrent = strstr($currentUrl,'?');
                if(strpos($currentUrl,'page')){
                    $url = strstr($currentUrl,'page',true)."?postyear=".$years[$i]->year."&postmonth=".$month->month;
                }else{
                    $url = strstr($currentUrl,'?',true)."?postyear=".$years[$i]->year."&postmonth=".$month->month;
                }
                ...
                if($paramCurrent!=false && $paramCurrent==strstr($url,'?')){
                    $expandMonth = 'active';
                }

                if($instance['only_sym_link']){
//                    $html.= "</a><a href=\"{$month_url}\">";
                    $html.= "</a><a class='".$expandMonth."' href='".$url."'>";
                }
