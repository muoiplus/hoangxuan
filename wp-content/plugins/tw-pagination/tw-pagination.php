...
if (!class_exists('TW_Pagination')) {
	class TW_Pagination {
		...
		function paginate($query = 'global', $args = false) {
			...
			if ($pages > 1) {
				...
				if ($page > 1 && ! empty ( $previouspage )) {
					$output .= sprintf ( '<li class="first"><a href="%s" class="prev"><span class="dart-narrow"><</span> %s</a></li>', $prevlink, stripslashes ( $previouspage ) );
				}elseif($page==1){
                    $output .= sprintf ( '<li class="first"><a href="%s" class="disable"><span class="dart-narrow"><</span> %s</a></li>', $prevlink, stripslashes ( $previouspage ) );
                }
                $output .= sprintf ( '<li><span>Page:</span></li>', stripslashes ( $title ) );
				$min_links = $range * 2 + 1;
				...
				if ($page < $pages && ! empty ( $nextpage )) {
					$output .= sprintf ( '<li class="last"><a href="%s" class="next"><span class="dart-narrow">></span> %s</a></li>', $nextlink, stripslashes ( $nextpage ) );
				}elseif($page==$pages){
                    $output .= sprintf ( '<li class="last"><a href="%s" class="disable"><span class="dart-narrow">></span> %s</a></li>', $nextlink, stripslashes ( $nextpage ) );
                }
				$output .= "</ol>";
			}
        ...
        function paginate_mobile($query = 'global', $args = false) {
            if ($this->type === 'comments' && ! get_option ( 'page_comments' ))
                return;
            ...
            if ($pages > 1) {
                ...
                $right_gap = (($block_high + $anchor + $gap) < $pages) ? true : false;
                if ($left_gap && ! $right_gap) {
                    $output .= sprintf ( '%s%s%s', $this->paginate_loop_mobile ( 1, $anchor ), $ellipsis, $this->paginate_loop_mobile ( $block_min, $pages, $page ) );
                } else if ($left_gap && $right_gap) {
                    $output .= sprintf ( '%s%s%s%s%s', $this->paginate_loop_mobile ( 1, $anchor ), $ellipsis, $this->paginate_loop_mobile ( $block_min, $block_high, $page ), $ellipsis, $this->paginate_loop_mobile ( ($pages - $anchor + 1), $pages ) );
                } else if ($right_gap && ! $left_gap) {
                    $output .= sprintf ( '%s%s%s', $this->paginate_loop_mobile ( 1, $block_high, $page ), $ellipsis, $this->paginate_loop_mobile ( ($pages - $anchor + 1), $pages ) );
                } else {
                    $output .= $this->paginate_loop_mobile ( 1, $pages, $page );
                }
                if ($page < $pages && ! empty ( $nextpage )) {
                    $output .= sprintf ( '<li class="last"><a href="%s" class="next"><span class="dart-narrow">></span> %s</a></li>', $nextlink, stripslashes ( $nextpage ) );
                }elseif($page==$pages){
        ...
        function paginate_loop_mobile($start, $max, $page = 0) {
            $output = "";
            for($i = $start; $i <= $max; $i ++) {
                $p = ($this->type === 'posts') ? esc_url ( get_pagenum_link ( $i ) ) : get_comments_pagenum_link ( $i );
                $output .= ($page == intval ( $i )) ? "<li><span class='page current'>$i of $max</span></li>" : "";
            }
            return $output;
        }
...
function tw_pagination_mobile($query = 'global', $args = false) {
    global $tw_pagination;
    return $tw_pagination->paginate_mobile ( $query, $args );
}
