...
if(isset($_SESSION['modeview'])){
    if(!isset($_GET['modeview'])){
        if(strpos($currentUrl,'?mem_categories')!=false){
            $newurl =$currentUrl.'&modeview='.$_SESSION['modeview'];
        }else{
            $newurl =$currentUrl.'?modeview='.$_SESSION['modeview'];
        }
...
<div class="inner">
      <h1 class="title title_mobile"><span></span> <?php single_cat_title('',true); ?></h1>
        <div class="left_sibar">
            <?php dynamic_sidebar( 'Left menu members' ); ?>
        </div>
  <div class="list_member">
  <h1 class="title title_desktop"><span></span> <?php single_cat_title('',true); ?></h1>
...
<div class="filters"><?php echo do_shortcode( '[searchandfilter taxonomies="mem_categories,states" headings="Showing,in" submit_label="Apply Filter" ]' ); ?>
  <div class="mode_view">
      ...
      ?>
      <a href="<?php if(strpos($url,'&modeview=list')!=false){echo strstr($url,'&modeview',true).'&modeview=grid';}else{echo $url.'?modeview=grid';}?>" id="gridview" class="<?php if($perPage==get_option('posts_per_page')){ echo 'active';}?>" title="view as grid">gird</a>
      <a href="<?php if(strpos($url,'?mem_categories')!=false){if(strpos($url,'&modeview')!=false){echo strstr($url,'&modeview',true).'&modeview=list';}else{echo $url.'&modeview=list';}}else{echo $url.'?modeview=list';}?>" id="listview" class="<?php if($perPage==15){ echo 'active';}?>" title="view as list">List</a>
    </div>
    <div class="clear"></div>
</div>
      ...
      <li class="<?php if(($i%3) == 0):?>last<?php elseif(($i%2) == 0):?>two<?php endif;?>">
        ...
        <div class="title">
          <h3> <a href="<?php the_permalink();?>" title="<?php the_title_attribute();?>">
           ...
           <a target="_blank" href="<?php $website = get_field('website');if (strpos($website,'http://') == false) {  echo 'http://';};the_field('website');?>"><?php the_field('website');?>
                    </a>
          </div>
           </div>
...
jQuery(document).ready(function() {
   var titleReseach = jQuery("#ofmem_categories option:selected").text();
        var allState = jQuery('#ofstates').val();
        var allResearch = jQuery('#ofmem_categories').val();
        if(allState!=0&&allResearch!=0){
            jQuery('.ourmember_listing h1.title span').text(titleReseach+' in ');
        }
});
</script>