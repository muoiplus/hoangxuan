...
<?php get_header(); 
$fullCurrentCat = explode('/', $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"]);
$currentCat = $fullCurrentCat[2];

$inCatAbout = array('about-us');
$inCatHM = array('health-medical-research');
$inCatMember = array('members','our-members');
$inCatNews = array('news-events','aamri-news','media-releases-news-events','news-clippings','newsletter','events');
$inCatPoAd = array('policy-advocacy','submissions','policy-papers');
$inCatPub = array('publications','reports','fact-sheets','corporate-documents');
?>
...
<div class="inner">
 ...
 <?php if ($currentCat && $currentCat != "") {
      ...
      if(in_array($currentCat, $inCatAbout)){
        dynamic_sidebar( 'Left menu About us' );
      }
      elseif (in_array($currentCat, $inCatHM)) {
         dynamic_sidebar( 'Health & Medical Research' );
      }
       elseif (in_array($currentCat, $inCatMember)) {
        dynamic_sidebar( 'Left menu members' );
      }
       elseif (in_array($currentCat, $inCatPoAd)) {
        dynamic_sidebar( 'Left menu Policy & Advocacy' ); 
      } elseif (in_array($currentCat, $inCatPub)) {
        dynamic_sidebar( 'Left menu Publications' ); 
      }elseif(in_array($currentCat, $inCatNews)){
