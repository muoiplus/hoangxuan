...
<div class="filters"><?php echo do_shortcode( '[searchandfilter taxonomies="mem_categories,states" headings="Showing,in" submit_label="Apply Filter" ]' ); ?>
	<div class="mode_view">
    <a href="/our-members-grid/" id="gridview" title="view as grid">gird</a>
        <a href="/our-members-list/" id="listview" class="active" title="view as list">List</a>
    </div>
    <div class="clear"></div>
</div>
  <div class="list_ourmember">
    <ul class="listmode" id="modeview">
    <?php $i=1 ?>
    <?php global $query_string; query_posts( $query_string . '&orderby=title&order=ASC' );
        $membersList = new WP_query();
        $membersList -> query(array('category_name'=>'our-members','posts_per_page'=>15,'paged'=>$paged));
      while($membersList->have_posts()): $membersList->the_post();
     ?>

      <li class="<?php if(($i%3) == 0):?>last <?php endif; if(($i%2) == 0):?>two <?php else: ?>one<?php endif;?>">

         <div class="thumb list"><?php echo $i;?><a href="<?php the_permalink();?>" title="<?php the_title_attribute();?>"> <?php the_post_thumbnail('member-thumb-list') ;?></a></div>
        <div class="title">
 ...
 <?php if(function_exists('tw_pagination')) 
    tw_pagination($membersList);
?>
    </div>
       <div class="paging mobile">
          <?php if(function_exists('tw_pagination'))
          tw_pagination_mobile($membersList);
          ?>
    </div>
